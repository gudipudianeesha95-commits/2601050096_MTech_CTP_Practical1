...
Each point contains an x-coordinate and a y-coordinate
...
Set the closest pair of points as empty.
Select the first point from the list.
...
If the calculated distance is smaller, update the minimum distance.
Store the two selected points as the closest pair.
...
Compare all pairs of points: O(n²)
Find minimum distance: O(1) for each comparison
